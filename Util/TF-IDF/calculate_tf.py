...
def main(args):
    ...
    for idx,wikipedia_data_dir in enumerate(tqdm(wikipedia_data_dirs)):
        ...
        with genkei_frequencies_file.open("w") as w:
            for genkei,freq in counter.most_common():
                w.write("{}\t{}\n".format(genkei,freq))

        total_num_words=len(genkeis)
        total_num_words_file:Path=wikipedia_data_dir.joinpath("total_num_words.txt")
        with total_num_words_file.open("w") as w:
            w.write(str(total_num_words))

        tf_file:Path=wikipedia_data_dir.joinpath("tf.txt")
        with tf_file.open("w") as w:
            for genkei,freq in counter.most_common():
                w.write("{}\t{}\n".format(genkei,freq/total_num_words))
